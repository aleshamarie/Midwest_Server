...
async function listOrdersDataTables(req, res) {
  try {
    ...
    if (searchValue) {
      searchQuery = {
        $or: [
          { order_code: { $regex: searchValue, $options: 'i' } },
          { name: { $regex: searchValue, $options: 'i' } },
          { contact: { $regex: searchValue, $options: 'i' } },
          { address: { $regex: searchValue, $options: 'i' } },
          { status: { $regex: searchValue, $options: 'i' } },
          { payment: { $regex: searchValue, $options: 'i' } }
        ]
      };
    }
    ...
    const filteredRecords = await Order.countDocuments(searchQuery);
    
    // Get paginated orders with search and sorting
    ...
    const data = orders.map(order => {
      ...
      return {
        DT_RowId: order._id,
        order_code: order.order_code || `ORD${order._id}`,
        name: order.name || '-',
        ...
        status: `<span class="px-2 py-1 rounded text-xs ${statusClass}">${statusIcon} ${order.status || '-'}</span>`,
        actions: `<button onclick="viewOrder('${order._id}')" class="text-blue-600 hover:text-blue-800">View</button> | <button onclick="editOrder('${order._id}')" class="text-green-600 hover:text-green-800">Edit</button> | <button onclick="deleteOrder('${order._id}')" class="text-red-600 hover:text-red-800">Delete</button>`
      };
    });
    ...
    console.error('Error in listOrdersDataTables:', error);
    res.status(500).json({ 
...
module.exports = { listOrders, listOrdersDataTables, updateOrderPayment, createOrder, getOrder, getOrderItems };